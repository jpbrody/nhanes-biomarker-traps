...
def main():
    ...
    df_val_a_clean['z_bin'] = df_val_a_clean['Z_master'].apply(get_z_bin)
    ...
    df_val_b_clean['z_bin'] = df_val_b_clean['Z_master'].apply(get_z_bin)
    ...
    bin_order = ["Z < -1.0", "-1.0 to -0.5", "-0.5 to 0.5", "0.5 to 1.0", "Z > 1.0"]
    custom_palette = ["#00d2ff", "#70b5ff", "#c293ff", "#ff73aa", "#ff5252"]
    
    # Plot Panel A (Left)
    ax_a = axes[0]
    sns.violinplot(
        x='z_bin', y='depression_score', data=df_val_a_clean,
        order=bin_order, palette=custom_palette, inner='quartile', ax=ax_a,
        cut=0, linewidth=1.5
    )
    
    # Calculate means and overlay
    means_a = [df_val_a_clean[df_val_a_clean['z_bin'] == b]['depression_score'].mean() for b in bin_order]
    ax_a.plot(range(5), means_a, color='#ffffff', linestyle='--', marker='o', markersize=8, linewidth=2.5, label='Empirical Mean PHQ-9', zorder=5)
    
    # Annotate depressed percentages directly above violins
    for i, b in enumerate(bin_order):
        subset = df_val_a_clean[df_val_a_clean['z_bin'] == b]
        rate = subset['depression_binary'].mean() * 100
        n_val = len(subset)
        ax_a.text(i, 25.5, f"Prev: {rate:.1f}%\n(n={n_val})", ha='center', va='bottom', fontsize=9.5, color='#ffffff', weight='bold')
        
    ax_a.set_title("Panel A: Historical Decadal Cohort (2009-2010)\n$N = 5,181$", pad=15)
    ax_a.set_xlabel("Master Neuro-Metabolic Score Z-Score Bins")
    ...
    ax_b = axes[1]
    sns.violinplot(
        x='z_bin', y='depression_score', data=df_val_b_clean,
        order=bin_order, palette=custom_palette, inner='quartile', ax=ax_b,
        cut=0, linewidth=1.5
    )
    
    # Calculate means and overlay
    means_b = [df_val_b_clean[df_val_b_clean['z_bin'] == b]['depression_score'].mean() for b in bin_order]
    ax_b.plot(range(5), means_b, color='#ffffff', linestyle='--', marker='o', markersize=8, linewidth=2.5, label='Empirical Mean PHQ-9', zorder=5)
    
    # Annotate depressed percentages directly above violins
    for i, b in enumerate(bin_order):
        subset = df_val_b_clean[df_val_b_clean['z_bin'] == b]
        rate = subset['depression_binary'].mean() * 100
        n_val = len(subset)
        ax_b.text(i, 25.5, f"Prev: {rate:.1f}%\n(n={n_val})", ha='center', va='bottom', fontsize=9.5, color='#ffffff', weight='bold')
        
    ax_b.set_title("Panel B: Post-Pandemic Modern Cohort (2021-2023)\n$N = 4,817$", pad=15)
    ax_b.set_xlabel("Master Neuro-Metabolic Score Z-Score Bins")
    ax_b.legend(facecolor='#181a20', edgecolor='#2a2d37', loc='upper left')
    
    plt.suptitle("Clinical Depression Distribution vs. Master Neuro-Metabolic Score\n(Side-by-Side Multi-Era External Validations)", y=0.98)
    plt.tight_layout()
    
    brain_dir = r"C:\Users\jpbro\.gemini\antigravity\brain\1663cb7c-e3b5-41f5-a60c-8dde1ab89813"
    plt.savefig(os.path.join(brain_dir, "depression_validation_violins.png"), dpi=300, facecolor='#111216')
    plt.close()
    
    print("\nSpectacular binned violin plot generated successfully at depression_validation_violins.png!")
    print("==================================================")
