...
def download_and_clean_recent():
    print("\n==================================================")
    print("Downloading and cleaning 2021-2023 Recent Cohort (_L)")
    print("==================================================")
    ...
    if len(existing_dep) == 9:
        print("Cleaning PHQ-9 items using float precision mapping...")
    ...
    print(f"Recent cohort clean shape: {merged_df.shape}")
...
def main():
    ...
    # Fit exact structural OLS models on Discovery Cohort
    ...
    # 2. Get and Clean Recent Cohort (2021-2023)
    df_val = download_and_clean_recent()
    df_val["const"] = 1.0
    
    # 3. Project Discovery OLS residuals onto Recent Cohort
    print("\nProjecting structural residuals onto 2021-2023 Validation Cohort...")
    df_val_clean = df_val[['depression_binary', 'BMXBMI', 'BMXWAIST', 'LBDHDD', 'LBXHSCRP', 'RIDAGEYR', 'RIAGENDR', 'const']].dropna().copy()
    
    # Predict Visceral Residual
    val_waist_pred = (
    ...
    # Predict CRP Residual (HSCRP_L is already in mg/L, so we use LBXHSCRP directly!)
    val_log_crp = np.log(df_val_clean["LBXHSCRP"] + 1e-3)
    val_log_crp_pred = (
    ...
    # 4. Construct Master Score using EXACT Discovery Weights
    ...
    print("\nEvaluating Master Score on post-pandemic 2021-2023 Validation Cohort...")
    val_model = sm.Logit(
    ...
    print("\n==================================================")
    print("Master Score 2021-2023 VALIDATION Results")
    print("==================================================")
    ...
    print("\nClinical Depression Rate by 2021-2023 Master Score Quintile:")
    for q in range(1, 6):
    ...
    sns.regplot(
        ...
        scatter_kws={"color": "#00d2ff", "alpha": 0.05, "s": 10},
        line_kws={"color": "#00d2ff", "linewidth": 3, "label": f"2021-2023 Risk Curve (z = {val_res.tvalues['Z_master']:.2f})"},
        ax=ax
    )
    ...
    plt.savefig(os.path.join(brain_dir, "depression_recent_validation_score.png"), dpi=300, facecolor='#111216')
    plt.close()
    
    print("\nMaster Score 2021-2023 VALIDATION Plot generated successfully!")
    print("==================================================")
